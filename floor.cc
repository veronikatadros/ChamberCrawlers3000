...
Floor::Floor() {
    for (int i = 0; i < 25; ++i) {
        for (int j = 0; j < 79; ++j) {
            if (j == 0 || j == 78) {
                board[i][j] = Cell(Cell::VWALL); // Vertical Boundary walls
            } 
            else if (i == 0 || i == 24){
                board[i][j] = Cell(Cell::HWALL);
            }
            else if ((i == 3 || i == 4) && (j >= 39 && j <= 61)){
                board[i][j] = Cell(Cell::GROUND);
            }
                ...
                board[i][j] = Cell(Cell::EMPTY);
            }
            else if (j == 1 || j == 77){
                board[i][j] = Cell(Cell::EMPTY);
            }
            else if (i >= 2 && i <= 7 && j == 2){
                board[i][j] = Cell(Cell::VWALL);
            }
